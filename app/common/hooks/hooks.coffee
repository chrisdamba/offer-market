...
#
OfferMarket.Collections.Packages.after.update (
	userId, doc, fieldNames, modifier, options) ->
	if modifier.$set?.settings?.mail or modifier.$set?['settings.mail.user']
		if Meteor.isServer
			OfferMarket.configureMailUrl() # set email configuration

#
# create unpublished product
#
OfferMarket.Collections.Products.before.insert (userId, product) ->
	# avoid calling if present
	product.shopId = product.shopId || OfferMarket.getShopId()
	_.defaults(product,
		productType: "Simple"
...
# On product update
###
OfferMarket.Collections.Products.before.update (
