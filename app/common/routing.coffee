...
ShopAdminController = @ShopController.extend
	onBeforeAction: () ->
		...
		unless OfferMarket.hasPermission @route.getName()
...
Router.map ->
	...
	@route 'dashboard/settings/shop',
		...
		data: ->
			OfferMarket.Collections.Shops.findOne()
	...
	@route 'dashboard/orders',
		...
		data: ->
			if Orders.findOne(@params._id)
				OfferMarket.Collections.Orders.findOne({'_id': @params._id})
		...
		data: ->
			if @ready()
				if Orders.findOne() or Meteor.userId()
					...
					OfferMarket.Collections.Orders.find({}, {sort: { createdAt: -1 }})
				else
					@render 'unauthorized'
			else
				@render "loading"
