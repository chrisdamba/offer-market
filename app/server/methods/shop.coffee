...
Meteor.methods
	...
	createShop: (userId, shop) ->
		...
		unless OfferMarket.hasOwnerAccess()
			throw new Meteor.Error 403, "Access Denied"
		@unblock()

		adminRoles  = Roles.getRolesForUser(currentUser, OfferMarket.getShopId())
		shopId = Factory.create("shop")._id
		try
			OfferMarket.Events.warn "Created shop: ", shopId
			Roles.addUsersToRoles [currentUser, userId], adminRoles, shopId
			shopId
		catch e
			OfferMarket.Events.warn "Failed to createShop", e

	###
	# determine user's countryCode and locale object
	###
	getLocale: ->
		...
		shop = OfferMarket.Collections.Shops.findOne OfferMarket.getShopId()
			...
			for currency in localeCurrency
				if shop.currencies[currency]
					...
					if shop.currency isnt currency
						...
						unless exchangeRate
							OfferMarket.Events.warn "Failed to fetch rate exchange rates."
						result.currency.exchangeRate = exchangeRate.data
					result #returning first match.
	...
	locateAddress: (latitude, longitude) ->
		...
		try
			...
			if latitude? and longitude?
				OfferMarket.Events.info "Error in locateAddress for latitude/longitude lookup (" + latitude + "," + longitude + "):" + error.message
			else
				OfferMarket.Events.info "Error in locateAddress for IP lookup (" + ip + "):" + error.message
	...
	updateHeaderTags: (tagName, tagId, currentTagId) ->
		...
		unless OfferMarket.hasPermission('core')
			throw new Meteor.Error 403, "Access Denied"
		@unblock()
		...
		if tagId
			Tags.update tagId, {$set:newTag}, () ->
				OfferMarket.Events.info "Changed name of tag " + tagId + " to " + tagName
				true
		# create a new tag
		else if existingTag
			...
			if currentTagId
				Tags.update currentTagId, {$addToSet: {"relatedTagIds": existingTag._id}}, () ->
					OfferMarket.Events.info 'Added tag "' + existingTag.name + '" to the related tags list for tag ' + currentTagId
					true
			else
				Tags.update existingTag._id, {$set:{"isTopLevel":true}}, () ->
					OfferMarket.Events.info 'Marked tag "' + existingTag.name + '" as a top level tag'
					true
			#if a tag with that name does not exist yet
		else
			newTag.isTopLevel = !currentTagId
			newTag.shopId = OfferMarket.getShopId()
			newTag.updatedAt = new Date()
			newTag.createdAt = new Date()
			newTagId = Tags.insert newTag
			if currentTagId
				Tags.update currentTagId, {$addToSet: {"relatedTagIds": newTagId}}, () ->
					OfferMarket.Events.info 'Added tag "' + newTag.name + '" to the related tags list for tag ' + currentTagId
					true
			else if newTagId && !currentTagId
				OfferMarket.Events.info 'Created tag "' + newTag.name + '"'
				true
			else
	...
	###
	flushTranslations: ->
		unless OfferMarket.hasAdminAccess()
			throw new Meteor.Error 403, "Access Denied"

		OfferMarket.Collections.Translations.remove({})
		Fixtures.loadI18n()
		OfferMarket.Events.info Meteor.userId() + " Flushed Translations."