...
@getCurrentCart = (sessionId, shopId, userId) ->
  ...
  shopid = shopId || OfferMarket.getShopId(@)
  ...
  if currentCarts.count() is 0
    ...
    OfferMarket.Events.debug "Created new session cart", newCartId
  ...
  currentCarts.forEach (cart) ->
    ...
    if cart.userId and !userId
      ...
      OfferMarket.Events.debug "Logging out. Removed session from cart."
    #
    ...
    if userId
      ...
      if userCart and !cart.userId
        ...
        OfferMarket.Events.debug "Updated user cart", cart._id, "with sessionId: " + sessionId
  ...
  if currentCarts.count() is 1
    ...
    OfferMarket.Events.debug "getCurrentCart returned sessionId:" + sessionId + " cartId: " + cart[0]._id
    currentCarts = Cart.find cart[0]._id
    return currentCarts

  # if all patterns failed.
  OfferMarket.Events.debug "getCurrentCart error:", currentCarts
  return currentCarts
...
Meteor.methods
  ...
  addToCart: (cartId, productId, variantData, quantity) ->
    ...
    check variantData, OfferMarket.Schemas.ProductVariant
    ...
    if cartVariantExists
      ...
      (error, result) ->
        if error
          OfferMarket.Events.warn "error adding to cart" , Cart.simpleSchema().namedContext().invalidKeys()
          return error
        return
    # add new cart items
    else
      product = OfferMarket.Collections.Products.findOne(productId)
  ...
  copyCartToOrder: (cartId) ->
    ...
    ###
    # final sanity check
    # TODO add `check cart, OfferMarket.Schemas.Order`
    ...
    try
      ...
      if orderId
        ...
        OfferMarket.Events.info "Completed cart for " + cartId

    catch error
      OfferMarket.Events.info "error in order insert"
      OfferMarket.Events.warn error, Orders.simpleSchema().namedContext().invalidKeys()
      return error
