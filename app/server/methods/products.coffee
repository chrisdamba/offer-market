Meteor.methods
	...
	cloneVariant: (productId, variantId, parentId) ->
		...
		false unless variant.length > 0

		clone = variant[0]
		clone._id = Random.id()

		if parentId
			OfferMarket.Events.debug "create child clone"
		...
		if children.length > 0
			OfferMarket.Events.debug "clone children"
	...
	createVariant: (productId, newVariant) ->
		...
		if newVariant
			...
			check(newVariant, OfferMarket.Schemas.ProductVariant)
		else
		...
		_.each deleted, (product) ->
			_.each product.variants, (variant) ->
				if variant.parentId is variantId or variant._id is variantId
					OfferMarket.Collections.Media.update(
	...
	cloneProduct: (product) ->
		...
		#check product, OfferMarket.Schemas.Product
		unless OfferMarket.hasPermission('createProduct')
			throw new Meteor.Error 403, "Access Denied"
		@unblock()
		...
		while i < product.variants.length
			...
			OfferMarket.Collections.Media.find(
		...
		Products.insert(product, {validate: false})
	...
	createProduct: (product) ->
		...
		if product
			Products.insert product # returns id
		else
			Products.insert
				_id: Random.id()
				title: ""
	...
	deleteProduct: (productId) ->
		...
		if numRemoved > 0
			# unlink media
			OfferMarket.Collections.Media.update 'metadata.productId': productId,
				$unset:
					'metadata.productId': ""
					'metadata.variantId': ""
					'metadata.priority': ""
	...
	publishProduct: (productId) ->
		...
		if product?.variants[0].price and
			...
			Products.findOne(product._id).isVisible
		else
			# debug log this
			OfferMarket.Events.debug "invalid product visibility ", productId
